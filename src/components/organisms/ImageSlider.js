  ...
  render() {
    ...
    if (images !== null) {
      ...
      settings = {
        customPaging: i => {
          return (
            <div className="slide__custom-dots">
              {i + 1}/{slideCount}
            </div>
          )
        },
      ...
      let renderedImages =
        slideCount === 1 ? slideImages : slideCount > 1 ? multipleImages : null

      return (
        <section className="section section__image-slider usa-grid">
          <div className="slide-images">{renderedImages}</div>
        </section>
      )
    } else {
      return null
